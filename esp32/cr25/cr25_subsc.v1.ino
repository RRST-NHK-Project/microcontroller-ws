...
float target_rpm[NUM_MOTORS] = {0};           // 目標速度
float pos_error_prev[NUM_MOTORS] = {0};       // 前回角度誤差
float vel_prop_prev[NUM_MOTORS] = {0};
float pos_integral[NUM_MOTORS] = {0};         // 角度積分項
float pos_output[NUM_MOTORS] = {0};           // PID出力
float vel_output[NUM_MOTORS] = {0};
float vel_out[NUM_MOTORS] = {0};
...
float kp_vel = 1.0f;  // 速度比例ゲイン
float ki_vel = 0.0f; // 速度積分ゲイン
float kd_vel = 0.05f; // 速度微分ゲイン
        ...
        while (packetSize) {
            ...
            if (id >= 0x201 && id < 0x201 + NUM_MOTORS) {
                ...
                if (!offset_ok[motor_index]) {
                    ...
                    vel_prop_prev[motor_index] = 0;
                    vel_out[motor_index] = 0;
